...
async function initOceanAndSky() {
    ...
    water = new THREE.Water(
        waterGeometry,
        {
            textureWidth: 512,
            textureHeight: 512,
            waterNormals: new THREE.TextureLoader().load('/public/textures/agua.jpg', function (texture) { texture.wrapS = texture.wrapT = THREE.RepeatWrapping; }),
...
function gameOver() {
    ...
    gameOverDiv.innerHTML = "VOCE PERDEU! </br> ESTE FOI SEU RESULTADO: " + torusScore;
...
function showInvertedControlsDiv() {
    const textIfTrue = "Controles invertidos: Ligado <br/> O avião seguirá o cursor do seu mouse. <br/> Esta configuração é salva no seu navegador e pode ser alternada pressionando 'I'.";
    const textIfFalse = "Controles invertidos: Desligado <br/> A direção do controle é realista, como em um avião real. <br/> Esta configuração é salva no seu navegador e pode ser alternada pressionando 'I'.";
...
function pauseGame() {
    ...
    if (isFlying) {
        isFlying = false;
        document.getElementById("time").innerHTML = "Pausado";
    } else {
        isFlying = true;
    }
}
